...
with st.expander("Theory & Formulas", expanded=False):
    st.markdown("### Ensemble Learning (Bagging)")
    st.markdown("A Random Forest is an ensemble of Decision Trees, usually trained with the **bagging** method. The general idea is that a combination of learning models increases the overall result.")
    st.markdown("### Aggregation")
    st.latex(r"\hat{f}(x) = \frac{1}{B} \sum_{b=1}^{B} f_b(x)")
    st.markdown("Where $B$ is the number of trees (`n_estimators`).")
...
n_estimators = st.sidebar.slider("Number of Trees", 1, 200, 10, 5, help="More trees usually reduce overfitting (up to a point).")
max_depth = st.sidebar.slider("Max Depth per Tree", 1, 20, 5)
...
if "2D" not in dataset_choice and feature_threshold > 0:
    temp_model = RandomForestClassifier(n_estimators=n_estimators, random_state=42).fit(X, y)
...
if dataset_choice == "Synthetic Classification (2D)":
    st.subheader(f"Decision Boundary (Trees={n_estimators}, Max Depth={max_depth})")
    fig = plot_decision_boundary(model, X, y, title=f"Random Forest Boundary")
    st.pyplot(fig)
else:
